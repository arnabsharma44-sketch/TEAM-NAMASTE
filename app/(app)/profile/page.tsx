...
export default function ProfilePage() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <div style={{ padding: 24, color: 'var(--text-dim)' }}>Loading profile...</div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <div style={{ maxWidth: 800, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
        <div>
          <Link href="/dashboard">
            <motion.div whileHover={{ x: -4 }} className="btn btn-ghost" style={{ marginBottom: 16, display: 'inline-flex' }}>
              <ArrowLeft size={16} /> Return Back
            </motion.div>
          </Link>
          <h1 className="font-display red-flicker" style={{ fontSize: 32, color: 'var(--red)', display: 'flex', alignItems: 'center', gap: 12 }}>
            <User size={32} /> Party Profile
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
          ...
          <motion.div
            ...
            whileHover={{ borderColor: 'var(--border-active)', boxShadow: '0 10px 30px rgba(0,0,0,0.6), 0 0 20px var(--red-glow)' }}
            className="card"
          ...
          <motion.div
            ...
            whileHover={{ borderColor: 'var(--red-primary)', boxShadow: '0 10px 30px rgba(0,0,0,0.6), 0 0 20px var(--red-glow-strong)' }}
            className="card"
            style={{ display: 'flex', flexDirection: 'column', gap: 16 }}
          >
            <h2 className="font-display" style={{ fontSize: 20, color: 'var(--red-primary)' }}>Party Member Synopsis</h2>
            ...
            {character ? (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', borderBottom: '1px solid var(--border)', paddingBottom: 12 }}>
                  ...
                  <div style={{ fontSize: 14, color: 'var(--red-primary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>{character.class}</div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div style={{ background: 'var(--bg)', padding: 12, borderRadius: 8 }}>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--gold)' }}>{character.gold} GP</div>
                  </div>
                </div>
